...
public class GithubIssueKohsukeService implements GithubIssueService {
    ...
    public NewLabelName createNewLabel(int year, int weekNumber) throws IOException {
        connectGithub();
        ...
        return new NewLabelName(ghLabel.getName());
    }
    ...
    public NewIssue createNewIssue(
        ...
        GHIssue ghIssue = repo.createIssue(issueTitle)
            .assignee(memberGithubName)
            .label(thisWeekNumberLabelName)
            .body(issueBody)
            .create();

        return new NewIssue(ghIssue.getNumber(), ghIssue.getTitle());
    }

    // Close 할 이슈 목록 가져오는 함수
    public List<IssueToClose> getIssuesToClose(String labelNameToClose) throws IOException {
        connectGithub();
        ...
        return repo.getIssues(GHIssueState.OPEN)
            ...
            .map(ghIssue -> new IssueToClose(ghIssue.getNumber(), ghIssue.getTitle()))
            .toList();
    }
