...
import static com.example.review_study_app.common.utils.MyWebclientUtils.createRetrySpec;
...
@Slf4j
@Service
public class GithubIssueWebclientService {
    ...
    @Autowired
    public GithubIssueWebclientService(
    ...
    public NewGithubLabel createNewLabel(int year, int weekNumber) throws Exception {
        ...
        RetryBackoffSpec retryBackoffSpec = createRetrySpec(3, Duration.ofSeconds(2));

        NewGithubLabel newGithubLabel = webClientHttpClient.post(request, NewGithubLabel.class).retryWhen(retryBackoffSpec).block();

        return newGithubLabel;
    }
